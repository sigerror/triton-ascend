...
// CHECK-SAME:   [[PARAM_0_:%.+]]: memref<?xbf16> {tt.tensor_kind = 0 : i32}, [[PARAM_1_:%.+]]: memref<?xbf16> {tt.tensor_kind = 0 : i32}, [[PARAM_2_:%.+]]: memref<?xbf16> {tt.tensor_kind = 1 : i32}, [[PARAM_3_:%.+]]: i32, [[PARAM_4_:%.+]]: i32, [[PARAM_5_:%.+]]: i32, [[PARAM_6_:%.+]]: i32, [[PARAM_7_:%.+]]: i32, [[PARAM_8_:%.+]]: i32)
// CHECK-SAME:    attributes {SyncBlockLockArgIdx = 0 : i64, WorkspaceArgIdx = 1 : i64, global_kernel = "", mix_mode = "mix"} {
...
// CHECK:           [[VAR_4_:%.+]] = linalg.matmul {input_precison = "ieee"} ins([[VAR_2_]], [[VAR_3_]] : tensor<128x64xbf16>, tensor<64x256xbf16>) outs([[VAR_1_]] : tensor<128x256xbf16>) -> tensor<128x256xbf16>
// CHECK:           bufferization.materialize_in_destination [[VAR_4_]] in writable [[VAR_reinterpret_cast_2_]]
