...
module {
		tt.func public @fn_npu_(%arg0: !tt.ptr {tt.divisibility = 16 : i32}, %arg1: !tt.ptr {tt.divisibility = 16 : i32}, %arg2: !tt.ptr {tt.divisibility = 16 : i32}) attributes {noinline = false} {
		...
		%36 = math.floor %35 : tensor<2x29x4xf8E4M3FN>
		%37 = arith.addf %32, %36 : tensor<2x29x4xf8E4M3FN>
		%38 = tt.splat %arg0 : !tt.ptr -> tensor<2x29x4x!tt.ptr>
		%39 = tt.addptr %38, %29 : tensor<2x29x4x!tt.ptr>, tensor<2x29x4xi32>
		tt.store %39, %37 : tensor<2x29x4x!tt.ptr>
		tt.return
	}
}

// CHECK: %[[ALLOC_INPUT1:[A-Za-z0-9_]+]] = memref.alloc() : memref<2x29x4xf8E4M3FN>
// CHECK: %[[TENSOR_INPUT1:[A-Za-z0-9_]+]] = bufferization.to_tensor %[[ALLOC_INPUT1]] restrict writable : memref<2x29x4xf8E4M3FN>

// CHECK: %[[ALLOC_INPUT2:[A-Za-z0-9_]+]] = memref.alloc() : memref<2x29x4xf8E4M3FN>
// CHECK: %[[TENSOR_INPUT2:[A-Za-z0-9_]+]] = bufferization.to_tensor %[[ALLOC_INPUT2]] restrict writable : memref<2x29x4xf8E4M3FN>
// CHECK: %[[FLOOR_RESULT:[A-Za-z0-9_]+]] = math.floor %[[TENSOR_INPUT2]] : tensor<2x29x4xf8E4M3FN>

// CHECK: %[[ADD_RESULT:[A-Za-z0-9_]+]] = arith.addf %[[TENSOR_INPUT1]], %[[FLOOR_RESULT]] : tensor<2x29x4xf8E4M3FN>
...
module {
		...
		%34 = tt.addptr %33, %29 : tensor<2x29x4x!tt.ptr>, tensor<2x29x4xi32>
		%35 = tt.load %34 : tensor<2x29x4x!tt.ptr>
		%36 = math.floor %35 : tensor<2x29x4xf8E5M2>
		%37 = arith.addf %32, %36 : tensor<2x29x4xf8E5M2>
		%38 = tt.splat %arg0 : !tt.ptr -> tensor<2x29x4x!tt.ptr>
		%39 = tt.addptr %38, %29 : tensor<2x29x4x!tt.ptr>, tensor<2x29x4xi32>
		tt.store %39, %37 : tensor<2x29x4x!tt.ptr>
		tt.return
...
// CHECK: %[[ALLOC_INPUT1:[A-Za-z0-9_]+]] = memref.alloc() : memref<2x29x4xf8E5M2>
// CHECK: %[[TENSOR_INPUT1:[A-Za-z0-9_]+]] = bufferization.to_tensor %[[ALLOC_INPUT1]] restrict writable : memref<2x29x4xf8E5M2>

// CHECK: %[[ALLOC_INPUT2:[A-Za-z0-9_]+]] = memref.alloc() : memref<2x29x4xf8E5M2>
// CHECK: %[[TENSOR_INPUT2:[A-Za-z0-9_]+]] = bufferization.to_tensor %[[ALLOC_INPUT2]] restrict writable : memref<2x29x4xf8E5M2>
// CHECK: %[[FLOOR_RESULT:[A-Za-z0-9_]+]] = math.floor %[[TENSOR_INPUT2]] : tensor<2x29x4xf8E5M2>

// CHECK: %[[ADD_RESULT:[A-Za-z0-9_]+]] = arith.addf %[[TENSOR_INPUT1]], %[[FLOOR_RESULT]] : tensor<2x29x4xf8E5M2>