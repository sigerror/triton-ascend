...
module {
		tt.func public @tt_clamp_3d(%arg0: !tt.ptr<f8E5M2> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f8E5M2> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %arg3: !tt.ptr<f32> {tt.divisibility = 16 : i32}) attributes {noinline = false} {
		%cst = arith.constant dense<4> : tensor<1x29x1xi32>
		%cst_0 = arith.constant dense<4> : tensor<2x1x1xi32>
		%cst_1 = arith.constant dense<29> : tensor<2x1x1xi32>
		%c4_i32 = arith.constant 4 : i32
		...
		%28 = tt.broadcast %26 : tensor<1x1x4xi32> -> tensor<2x29x4xi32>
		%29 = arith.addi %27, %28 : tensor<2x29x4xi32>
		%30 = tt.splat %arg0 : !tt.ptr<f8E5M2> -> tensor<2x29x4x!tt.ptr<f8E5M2>>
		...
		%40 = tt.clampf %39, %35, %38, propagateNan = none : tensor<2x29x4xf32>
		%41 = tt.splat %arg1 : !tt.ptr<f8E5M2> -> tensor<2x29x4x!tt.ptr<f8E5M2>>
		%42 = tt.addptr %41, %29 : tensor<2x29x4x!tt.ptr<f8E5M2>>, tensor<2x29x4xi32>
		%43 = tt.fp_to_fp %40, rounding = rtne : tensor<2x29x4xf32> -> tensor<2x29x4xf8E5M2>
		tt.store %42, %43 : tensor<2x29x4x!tt.ptr<f8E5M2>>
		tt.return
	}
}

// CHECK: %[[CAST_F8:[A-Za-z0-9_]+]] = memref.reinterpret_cast %arg2 to offset: [%9], sizes: [2, 29, 4], strides: [116, 4, 1] : memref<?xf8E5M2> to memref<2x29x4xf8E5M2, strided<[116, 4, 1], offset: ?>>
...
// CHECK: %[[CAST_MIN:[A-Za-z0-9_]+]] = memref.reinterpret_cast %arg4 to offset: [%9], sizes: [2, 29, 4], strides: [116, 4, 1] : memref<?xf32> to memref<2x29x4xf32, strided<[116, 4, 1], offset: ?>>
...
// CHECK: %[[CAST_MAX:[A-Za-z0-9_]+]] = memref.reinterpret_cast %arg5 to offset: [%9], sizes: [2, 29, 4], strides: [116, 4, 1] : memref<?xf32> to memref<2x29x4xf32, strided<[116, 4, 1], offset: ?>>
...
// CHECK: %[[F8_TO_F32:[A-Za-z0-9_]+]] = arith.extf %[[TENSOR_F8]] {round_mode = #hfusion.round_mode<rint>} : tensor<2x29x4xf8E5M2> to tensor<2x29x4xf32>
...
// CHECK: %[[CAST_OUTPUT:[A-Za-z0-9_]+]] = memref.reinterpret_cast %arg3 to offset: [%9], sizes: [2, 29, 4], strides: [116, 4, 1] : memref<?xf8E5M2> to memref<2x29x4xf8E5M2, strided<[116, 4, 1], offset: ?>>
// CHECK: %[[F32_TO_F8:[A-Za-z0-9_]+]] = arith.truncf %[[CLAMP_RESULT]] {round_mode = #hfusion.round_mode<rint>} : tensor<2x29x4xf32> to tensor<2x29x4xf8E5M2>
// CHECK: bufferization.materialize_in_destination %[[F32_TO_F8]] in writable %[[CAST_OUTPUT]] : (tensor<2x29x4xf8E5M2>, memref<2x29x4xf8E5M2, strided<[116, 4, 1], offset: ?>>) -> ()