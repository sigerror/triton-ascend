...
template <typename T> void PrintFormatString(int8_t *&buf, int8_t *maxbuf, T param)
{
    // 读取长度
    short len = *reinterpret_cast<short *>(buf);
    buf += sizeof(len);
...
void OnHostInitialize(PrintPayloadData *PrintData, unsigned BlockNum)
{
    ...
    int WholeSize = (PrintData->LogBufferSize + LogBufferPaddingBytes) * PrintData->BlockNum * NumPhysicalPerLogical;
...
void OnHostFinish(PrintPayloadData *PrintData, rtStream_t Stream)
{
    ...
    error =
        rtMemcpyAsync(hostMemOut2, WholeSize, PrintData->LogWholeRegion, WholeSize, RT_MEMCPY_DEVICE_TO_HOST, Stream);
    if (error != RT_ERROR_NONE) {
...
void Close(DebugTunnelData *DTData, rtStream_t Stream)
{
    ...
    error = rtMemcpy(&debugTunnelDataForHost, sizeof(debugTunnelDataForHost), DTData, sizeof(debugTunnelDataForHost),
