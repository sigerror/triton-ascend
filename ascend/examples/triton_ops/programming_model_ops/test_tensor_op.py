...
def triton_kernel(
    ...
    :param in0_stride1: 
    :param in0_stride2: 
    :param in0_stride_order1: 
    :param in0_stride_order2: 
    :param out0_stride1: 
    :param out0_stride2: 
    :param out0_stride_order1: 
    :param out0_stride_order2: 
    :param s1: 
    :param s2: 
    :param tile_size0: 
    :param tile_size1: 
    ...
    in0_bptr = tl.make_block_ptr(in0_ptr, (s0, s1, s2),
    ...
    in0 = tl.load(in0_bptr,
                  boundary_check=(in0_stride_order0, in0_stride_order1,
                                  in0_stride_order2)).to(
                                      in0_ptr.type.element_ty)

    out0 = in0

    out0_bptr = tl.make_block_ptr(
        out0_ptr, (s0, s1, s2), (out0_stride0, out0_stride1, out0_stride2),
...
def triton_func(in0: torch.Tensor, permute_order):
    ...
    shape = (in0_permuted_shape[0], in0_permuted_shape[1],
             in0_permuted_shape[2])
    tile_sizes = (shape[0], shape[1], shape[2])
    out0 = torch.empty(shape, dtype=in0.dtype, device=in0.device)
    out0_strides = out0.stride()
    out0_stride_order = [
        len(permute_order) - 1 - i
        for i in range(len(permute_order))
    ]
    grid = (shape[0] // tile_sizes[0], shape[1] // tile_sizes[1],
            shape[2] // tile_sizes[2])
    triton_kernel[grid](
        in0,
        out0,
...
def test_tensor(shape, permute_order):
    """_summary_
