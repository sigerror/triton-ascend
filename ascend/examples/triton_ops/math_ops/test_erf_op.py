...
@triton.jit
def triton_erf(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_erf(sigtype, L, M, N):
    ...
    if sigtype == "float16":
        y_ref = torch.erf(x0.to(torch.float32)).to(torch.float16)
    elif sigtype == "bfloat16":
        y_ref = torch.erf(x0.to(torch.float32)).to(torch.bfloat16)
    else:
        y_ref = torch.erf(x0)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_erf[1, 1, 1](x0, output, L, M, N)
