...
@triton.jit
def triton_log2(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_log2(sigtype, L, M, N):
    ...
    x0 = test_common.fill_negative_with_one(x0)
    y_ref = torch.log2(x0)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_log2[1, 1, 1](x0, output, L, M, N)
