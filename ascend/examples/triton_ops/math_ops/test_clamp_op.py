...
@triton.jit
def triton_clamp(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_clamp(sigtype, L, M, N):
    ...
    if "bool" == sigtype:
        x0 = x0.to(torch.int8)
        y_ref = torch.clamp(x0, min=0.0, max=100.0)
        y_ref = y_ref.to(dtype)
    else:
        y_ref = torch.clamp(x0, min=0.0, max=100.0)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_clamp[1, 1, 1](x0, output, L, M, N)
