...
@triton.jit
def triton_exp2(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_exp2(sigtype, L, M, N):
    ...
    y_ref = torch.pow(2, x0, out=None)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_exp2[1, 1, 1](x0, output, L, M, N)
