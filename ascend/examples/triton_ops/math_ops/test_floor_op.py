...
@triton.jit
def triton_floor(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_floor(sigtype, L, M, N):
    ...
    y_ref = torch.floor(x0)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_floor[1, 1, 1](x0, output, L, M, N)
