...
@triton.jit
def triton_neg(in_ptr0, out_ptr0, L: tl.constexpr, M: tl.constexpr, N: tl.constexpr):
    """
...
def test_neg(sigtype, L, M, N):
    ...
    elif "bool" in sigtype:
        y_ref = x0.logical_not_()
    else:
        y_ref = -x0
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_neg[1, 1, 1](x0, output, L, M, N)
