...
def test_rsqrt(sigtype, L, M, N):
    ...
    x1 = test_common.fill_negative_with_one(x1)
    if sigtype == "float16":
        y_ref = (x0.to(torch.float32) + 1.0 / (torch.sqrt(x1.to(torch.float32)))).to(
            torch.float16
        )
        ...
        y_ref = x0 + 1.0 / (torch.sqrt(x1))
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_rsqrt[1, 1, 1](x0, x1, output, L, M, N)
