...
def test_gt(sigtype, L, M, N):
    ...
    y_ref = torch.where(torch.gt(x0, x1), torch.ones_like(x0), torch.zeros_like(x0)).to(
        eval("torch." + sigtype)
    )
    logger.debug("torch result:")
    logger.debug(y_ref)
    output = torch.zeros(shape, dtype=dtype).npu()
    triton_gt[1, 1, 1](x0, x1, output, L, M, N)
    logger.debug("triton result:")
    logger.debug(output)
