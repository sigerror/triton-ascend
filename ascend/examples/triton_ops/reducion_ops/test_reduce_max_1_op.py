...
def triton_max_dim1(
    ...
    ret, ret1 = tl.reduce((x, x1), 1, maximum_with_index)
...
def test_reduce_max_1(sigtype, L, M, N):
    """
    ...
    if "int" in sigtype:
        ans, ans1 = torch.max(x0.to(torch.int64), 1)
        ans = ans.to(dtype)
    else:
        ans, ans1 = torch.max(x0, 1)
    output = torch.zeros((L, N), dtype=dtype).npu()
    output1 = torch.zeros((L, N), dtype=torch.int32).npu()
    triton_max_dim1[1, 1, 1](x0, x1, output, output1, L, M, N)
