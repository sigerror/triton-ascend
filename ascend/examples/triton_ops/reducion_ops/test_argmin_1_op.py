...
def test_argmin_1(sigtype, L, M, N):
    """
    ...
    if "int" in sigtype:
        ans = torch.min(x0.to(torch.int64), 1)[1]
    else:
        ans = torch.min(x0, 1)[1]
    output = torch.zeros((L, N), dtype=torch.int32).npu()
    triton_argmin_dim1[1, 1, 1](x0, output, L, M, N)
