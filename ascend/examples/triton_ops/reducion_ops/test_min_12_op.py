...
def test_min_12(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.min(x0.to(torch.int64), 1)[0].to(dtype)
        ans = torch.min(ans.to(torch.int64), 1)[0].to(dtype)
    else:
        ans = torch.min(x0, 1)[0]
        ans = torch.min(ans, 1)[0]

    output = torch.zeros((L,), dtype=dtype).npu()
    triton_min_dim1_dim2[1, 1, 1](x0, output, L, M, N)
