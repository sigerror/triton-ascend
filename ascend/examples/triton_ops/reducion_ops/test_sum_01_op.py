...
def test_sum_01(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.sum(x0.to(torch.int64), 0).to(dtype)
        ans = torch.sum(ans.to(torch.int64), 0).to(dtype)
    else:
        ans = torch.sum(x0, 0)
        ans = torch.sum(ans, 0)
    output = torch.zeros((N,), dtype=dtype).npu()
    triton_sum_dim0_dim1[1, 1, 1](x0, output, L, M, N)
