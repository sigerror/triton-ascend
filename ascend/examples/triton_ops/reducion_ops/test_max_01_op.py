...
def test_max_01(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.max(x0.to(torch.int64), 0)[0].to(dtype)
        ans = torch.max(ans.to(torch.int64), 0)[0].to(dtype)
    else:
        ans = torch.max(x0, 0)[0]
        ans = torch.max(ans, 0)[0]
    output = torch.zeros((N,), dtype=dtype).npu()
    triton_max_dim0_dim1[1, 1, 1](x0, output, L, M, N)
