...
def test_max_12(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.max(x0.to(torch.int64), 1)[0].to(dtype)
        ans = torch.max(ans.to(torch.int64), 1)[0].to(dtype)
    else:
        ans = torch.max(x0, 1)[0]
        ans = torch.max(ans, 1)[0]
    output = torch.zeros((L,), dtype=dtype).npu()
    triton_max_dim1_dim2[1, 1, 1](x0, output, L, M, N)
