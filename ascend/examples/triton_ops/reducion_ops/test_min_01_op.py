...
def test_min_01(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.min(x0.to(torch.int64), 0)[0].to(dtype)
        ans = torch.min(ans.to(torch.int64), 0)[0].to(dtype)
    else:
        ans = torch.min(x0, 0)[0]
        ans = torch.min(ans, 0)[0]
    output = torch.zeros((N,), dtype=dtype).npu()
    triton_min_dim0_dim1[1, 1, 1](x0, output, L, M, N)
