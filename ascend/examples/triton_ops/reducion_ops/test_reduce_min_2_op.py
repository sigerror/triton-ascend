...
def triton_min_dim2(
    ...
    ret, ret1 = tl.reduce((x, x1), 2, minimum_with_index)
...
def test_reduce_min_2(sigtype, L, M, N):
    """
    ...
    if "int" in sigtype:
        ans, ans1 = torch.min(x0.to(torch.int64), 2)
        ans = ans.to(dtype)
    else:
        ans, ans1 = torch.min(x0, 2)
    output = torch.zeros((L, M), dtype=dtype).npu()
    output1 = torch.zeros((L, M), dtype=torch.int32).npu()
    triton_min_dim2[1, 1, 1](x0, x1, output, output1, L, M, N)
