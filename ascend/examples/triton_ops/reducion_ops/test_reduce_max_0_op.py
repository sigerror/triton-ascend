...
def triton_max_dim0(
    ...
    ret, ret1 = tl.reduce((x, x1), 0, maximum_with_index)
...
def test_reduce_max_0(sigtype, L, M, N):
    """
    ...
    if "int" in sigtype:
        ans, ans1 = torch.max(x0.to(torch.int64), 0)
        ans = ans.to(dtype)
    else:
        ans, ans1 = torch.max(x0, 0)
    output = torch.zeros((M, N), dtype=dtype).npu()
    output1 = torch.zeros((M, N), dtype=torch.int32).npu()
    triton_max_dim0[1, 1, 1](x0, x1, output, output1, L, M, N)
