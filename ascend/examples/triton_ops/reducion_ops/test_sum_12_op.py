...
def test_sum_12(sigtype, L, M, N):
    ...
    if "int" in sigtype:
        ans = torch.sum(x0.to(torch.int64), 1).to(dtype)
        ans = torch.sum(ans.to(torch.int64), 1).to(dtype)
    else:
        ans = torch.sum(x0, 1)
        ans = torch.sum(ans, 1)
    output = torch.zeros((L,), dtype=dtype).npu()
    triton_sum_dim1_dim2[1, 1, 1](x0, output, L, M, N)
