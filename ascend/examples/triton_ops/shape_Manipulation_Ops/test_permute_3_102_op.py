...
@triton.jit
def triton_permute_3_102(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
    ...
    X = tl.load(x_ptr + idx)
    ret = X.permute(1, 0, 2)
    oidx = yidx[:, None, None] * XB * ZB + xidx[None, :, None] * ZB + zidx[
        None, None, :]
...
def test_permute_3_102(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.permute(1, 0, 2)
    output = torch.zeros((YB, XB, ZB), dtype=dtype).npu()
    triton_permute_3_102[1, 1, 1](output, x0, XB, YB, ZB)
