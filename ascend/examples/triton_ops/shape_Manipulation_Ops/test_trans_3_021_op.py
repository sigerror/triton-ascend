...
@triton.jit
def triton_trans_3_021(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                       ZB: tl.constexpr):
    """
    _summary_
...
def test_trans_3_021(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = torch.transpose(x0, 1, 2)
    output = torch.zeros((XB, ZB, YB), dtype=dtype).npu()
    triton_trans_3_021[1, 1, 1](output, x0, XB, YB, ZB)
