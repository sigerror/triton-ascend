...
@triton.jit
def triton_trans_3_120(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                       ZB: tl.constexpr):
    """
    _summary_
    ...
    X = tl.load(x_ptr + idx)
    ret = tl.trans(X, 1, 2, 0)
    oidx = yidx[:, None, None] * XB * ZB + zidx[None, :, None] * XB + xidx[
        None, None, :]
...
def test_trans_3_120(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.permute(1, 2, 0)
    output = torch.zeros((YB, ZB, XB), dtype=dtype).npu()
    triton_trans_3_120[1, 1, 1](output, x0, XB, YB, ZB)
