...
@triton.jit
def triton_expand_dims_0(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
    ...
    X = tl.load(x_ptr + idx)
    ret = tl.expand_dims(X, 0)
    expandx = tl.arange(0, 1)
    oidx = expandx[:, None, None, None] * XB * YB * ZB + xidx[
        None, :, None, None] * YB * ZB + yidx[None, None, :, None] * ZB + zidx[
            None, None, None, :]
...
def test_expand_dims_0(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.unsqueeze(0)
    output = torch.zeros((1, XB, YB, ZB), dtype=dtype).npu()
    triton_expand_dims_0[1, 1, 1](output, x0, XB, YB, ZB)
