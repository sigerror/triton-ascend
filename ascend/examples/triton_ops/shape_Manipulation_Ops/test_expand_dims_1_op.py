...
@triton.jit
def triton_expand_dims_1(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
    ...
    oidx = xidx[:, None, None, None]*YB*ZB+expandx[None, :, None, None] * \
        YB*ZB+yidx[None, None, :, None]*ZB+zidx[None, None, None, :]
...
def test_expand_dims_1(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.unsqueeze(1)
    output = torch.zeros((XB, 1, YB, ZB), dtype=dtype).npu()
    triton_expand_dims_1[1, 1, 1](output, x0, XB, YB, ZB)
