...
@triton.jit
def triton_broadcast_to_dim02(in_ptr0, out_ptr0, L: tl.constexpr,
    ...
    idx = tl.arange(
        0, 1)[:, None, None] * M * 1 + mblk_idx[None, :, None] * 1 + tl.arange(
            0, 1)[None, None, :]
...
def test_broadcast_to_02(sigtype, L, M, N):
    """
    _summary_
    ...
    x0 = test_common.generate_tensor(shape=(1, M, 1), dtype=sigtype).npu()
    ans = x0.repeat(L, 1, N)
