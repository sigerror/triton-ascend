...
@triton.jit
def triton_expand_dims_2(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
    ...
    oidx = xidx[:, None, None, None]*YB*ZB+yidx[None, :, None, None] * \
        ZB+expandx[None, None, :, None]+zidx[None, None, None, :]
...
def test_expand_dims_2(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.unsqueeze(2)
    output = torch.zeros((XB, YB, 1, ZB), dtype=dtype).npu()
    triton_expand_dims_2[1, 1, 1](output, x0, XB, YB, ZB)
