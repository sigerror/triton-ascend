...
@triton.jit
def triton_expand_dims_3(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
    ...
    X = tl.load(x_ptr + idx)
    ret = tl.expand_dims(X, 3)
    expandx = tl.arange(0, 1)
    oidx = xidx[:, None, None, None]*YB*ZB+yidx[None, :, None, None] * \
        ZB+zidx[None, None, :, None]+expandx[None, None, None, :]
...
def test_expand_dims_3(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.unsqueeze(3)
    output = torch.zeros((XB, YB, ZB, 1), dtype=dtype).npu()
    triton_expand_dims_3[1, 1, 1](output, x0, XB, YB, ZB)
