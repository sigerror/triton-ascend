...
@triton.jit
def triton_broadcast_to_dim2(in_ptr0, out_ptr0, L: tl.constexpr,
    ...
    idx = lblk_idx[:, None, None] * 1 * M + mblk_idx[None, :,
                                                     None] * 1 + tl.arange(
                                                         0, 1)[None, None, :]
...
def test_broadcast_to_2(sigtype, L, M, N):
    """
    _summary_
    ...
    x0 = test_common.generate_tensor(shape=(L, M, 1), dtype=sigtype).npu()
    ans = x0.repeat(1, 1, N)
