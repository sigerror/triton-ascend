...
@triton.jit
def triton_permute_3_210(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr,
                         ZB: tl.constexpr):
    """
    _summary_
...
def test_permute_3_210(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    ans = x0.permute(2, 1, 0)
    output = torch.zeros((ZB, YB, XB), dtype=dtype).npu()
    triton_permute_3_210[1, 1, 1](output, x0, XB, YB, ZB)
