...
@triton.jit
def triton_trans_2(output_ptr, x_ptr, XB: tl.constexpr, YB: tl.constexpr):
    """
    _summary_
...
def test_trans_2(sigtype, XB, YB, ZB):
    """
    _summary_
    ...
    x0 = test_common.generate_tensor(shape=(XB, YB), dtype=sigtype).npu()
    ans = torch.transpose(x0, 0, 1)
    output = torch.zeros((YB, XB), dtype=dtype).npu()
    triton_trans_2[1, 1, 1](output, x0, XB, YB)
