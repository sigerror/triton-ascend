...
@triton.jit
def triton_broadcast_to_dim12(in_ptr0, out_ptr0, L: tl.constexpr,
    ...
    idx = lblk_idx[:, None, None]*1*1 + \
        tl.arange(0, 1)[None, :, None]*1+tl.arange(0, 1)[None, None, :]
...
def test_broadcast_to_12(sigtype, L, M, N):
    """
    _summary_
    ...
    x0 = test_common.generate_tensor(shape=(L, 1, 1), dtype=sigtype).npu()
    ans = x0.repeat(1, M, N)
