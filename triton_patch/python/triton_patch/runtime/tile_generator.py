...
from .utils import byte_per_numel, next_power_of_2, num_vector_core
    ...
    def __init__(
        ...
        for name, length in axis_sizes.items():
            ...
            axis_dict[name] = AxisInfo(
                name=name, index=idx, length=length, prefix=prefix,
                split_name=split_name, tiling_name=tiling_name,
                is_split_axis=is_split_axis, is_tiling_axis=is_tiling_axis
            )
            idx += 1
...
class TileGenerator:
    ...
    def __init__(self, kernel_meta: KernelMeta):
        ...
        for axis in self.kernel_meta.axis_info:
            if axis.is_split_axis:
                self.split_magic_nums.append((axis.length + num_vector_core - 1) // num_vector_core)
            else:
                self.split_magic_nums.append(-1)
        
        self.candidates_blk_sizes: List[Tuple[int, ...]] = []
        self.configs = {}
        self.dtype_bytes = self.get_byte_per_numel(kernel_meta.dtype)
    ...
    @classmethod
    def get_byte_per_numel(cls, dtype: torch.dtype) -> int:
    ...
    def calculate_total_numel(self) -> int:
        total_numel = 1
        for axis in self.kernel_meta.axis_info:
            if axis.is_tiling_axis:
                total_numel *= self.blocks[axis.index].sub_block_size
            else:
                # this axis's tiling size is the same as block size
                total_numel *= self.blocks[axis.index].block_size
    ...
    def add_to_configs(self, cand_sizes) -> None:
        ...
        for axis in self.kernel_meta.axis_info:
            ...
            if axis.is_tiling_axis:
                kwargs[block_info.sub_block_name] = next_power_of_2(block_info.sub_block_size)
        ...
        for axis in self.kernel_meta.axis_info:
            ...
            if axis.is_tiling_axis:
                total_numel *= kwargs.get(self.blocks[axis.index].sub_block_name, 1)
            else:
                total_numel *= kwargs.get(self.blocks[axis.index].block_name, 1)

        key = self.get_key_from_dict(kwargs)
        if self.valid_tile_numel(total_numel) and key not in self.configs:
            self.configs[key] = Config(kwargs, num_warps=self.num_warps, num_stages=self.num_stages)
        ...
        while True:
            ...
            if is_split:
                ...
                if total_programs > num_vector_core // 2 or self.dual_reduction:
                    ...
                    self.candidates_blk_sizes.append(tuple([x.block_size for x in self.blocks]))

                if not magic_descend_split and (numel // 2) < self.split_magic_nums[axis_idx]:
                    self.blocks[axis_idx].block_size = self.split_magic_nums[axis_idx]
                    magic_descend_split = True
                else:
                    step = numel // 4 if numel // 4 > 1 else 1
                    self.blocks[axis_idx].block_size = numel // 2 if not slow_descend_split else numel - step
    ...
    def descend_all_low_dims(self) -> None:
        ...
        while total_numel > self.stop_numel and count < 100:
            ...
            new_total_numel = self.calculate_total_numel()
            if total_numel == new_total_numel:
                descend_all_axis(0)
        ...
        def descend_tiling_not_low_dims():
            for axis in tiling_not_low_dims:
                ...
                if self.descend_one_axis(axis.index):
                    return True
            return self.calculate_total_numel() <= self.stop_numel
